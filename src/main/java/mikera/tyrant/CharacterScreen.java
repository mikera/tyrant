...
public class CharacterScreen extends Screen {
	...
	public void paint(Graphics g) {
		...
		g.drawString("Skill Points  : " + h.getStat(RPG.ST_SKILLPOINTS), 40,
				40 + 16 * 16);
		g.drawString("Hunger level  : " + Text.properCase( Hero.hungerString(h) ) , 40 , 
		    40 + 17 * 16);
...
if (Game.isDebug()) {
		...
		g.drawString("Encumberance    : "
				+ Maths.middle(0, h.getStat(RPG.ST_ENCUMBERANCE), 100)
				+ "%", 240, 40 + 15 * 16);
		g.drawString("Wealth          : "
		...
		for (int i=0 ; i<al.size(); i++) {
		  ...
		  if( Skill.isAbility(s_core) ){
			  g.drawString(s, 460, 40 + skillLines * 16 + 4 + 20 + abilityLines * 16 );
			  abilityLines++;
		  }
		}		
