...
class RogueDungeon {
	...
	public static Map create(int l, Theme theme) {
			RogueDungeon rd = new RogueDungeon(
					3 + Rand.r(l / 3), 
					3 + Rand.r(l / 3), 
					8 + Rand.d(l), 
					8 + Rand.d(l));
	...
	public void build(Map m) {
		...
		for (int x = 0; x < bw; x++)
			for (int y = 0; y < bh; y++) {
				...
				rx[x][y] = x * tw + Rand.d(tw - rw[x][y] - 2);
				ry[x][y] = y * th + Rand.d(th - rh[x][y] - 2);
			}
		...
		for (int x = 0; x < (bw - 1); x++)
			for (int y = 0; y < bh; y++) {
				if ((links[x][y] & LE) > 0) {
					int py1 = ry[x][y] + Rand.r(rh[x][y]);
					int py2 = ry[x + 1][y] + Rand.r(rh[x + 1][y]);
		...
		for (int x = 0; x < bw; x++)
			for (int y = 0; y < (bh - 1); y++) {
				if ((links[x][y] & LS) > 0) {
					int px1 = rx[x][y] + Rand.r(rw[x][y]);
					int px2 = rx[x][y + 1] + Rand.r(rw[x][y + 1]);
