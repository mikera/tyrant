	...
	public static Script damage(String dt, int dam, String desc, int prob) {
		Script s=new Script() {
            ...
            public boolean handle(Thing t, Event e) {
				...
				if (Rand.r(100)>=prob) return false;
	...
	public static Script statGain(String targetProperty, String stats, int max, int chance) {
		Script sc=new Script() {
            ...
            public boolean handle(Thing t, Event e) {
				...
				String stat=ss[Rand.r(ss.length)];
    ...
    public static Script heal(String targetProperty, int amount, int chance) {
        Script script = new Script() {
            ...
            public boolean handle(Thing t, Event e) {
                ...
                int gain = Maths.min(amount, hpsmax - hps);
                if (gain > 0) {
                    if (gain > hps) {
			...
			if (gr > 0) {
				...
				for (int i = RPG.po(time * gr,1000000); i > 0; i--) {
					...
					if (s == null) {
						...
						String[] ss = s.split(",");
						nt = Lib.create(ss[Rand.r(ss.length)],level);
                    }
                    boolean added = false;
                    if (inPlace > 0) {
