...
public class Outdoors {
	...
	public static void buildOutdoors(Map m, int x1, int y1, int x2, int y2,
		...
		switch (t) {
			case Tile.PLAINS : {
				...
				for (int i = 0; i < (area / (20+Rand.d(100))); i++) {
				...
				if (Rand.d(6+Game.hero().getStat(Skill.HERBLORE))>=6) {
					Thing herb=Lib.createType("IsHerb",Rand.d(10));
					int r=7;
					int px=RPG.rspread(x1,x2-r);
					int py=RPG.rspread(y1,y2-r);
					for (int x=px; x<px+r; x++) {
						for (int y=py; y<py+r; y++) {
							if ((Rand.d(5)==1)&&(m.getTile(x,y)==m.floor())) {
			...
			case Tile.HILLS : {
				...
				for (int i = 0; i < (area / 10); i++) {
					m.setTile(x1 + Rand.r(w), y1 + Rand.r(h), Tile.HILLS);
				}
			...
			case Tile.FORESTS : {
				...
				if (Rand.d(6+Game.hero().getStat(Skill.FARMING))>=6) {
					m.incStat("Level",Rand.r(6));
					String st="mushroom";
					switch (Rand.d(6)) {
						case 1: 
							st="plant"; 
					...
					for (int i = 0; i < (area / (1+Rand.d(100))); i++) {
		...
		// add animals
		int animals=Rand.r(2+Game.hero().getStat(Skill.TRACKING)*2);
	...
	public static void addBeasties(Map map, int x, int y) {
		...
		for (int i = Rand.d(m.getStat("GroupNumber")); i > 0; i--) {
			int bx = x - 2 + Rand.r(5);
			int by = y - 2 + Rand.r(5);
