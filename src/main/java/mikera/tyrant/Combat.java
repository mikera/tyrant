...
public class Combat {
	...
	public static int calcAttackSpeed(Thing t) {
		...
		if (fer>0) {
			ms+=Maths.min(100,t.getStat("SK"))*fer/4;
		}
		return ms;
	}
	...
	public static int attack(Thing a, Thing target) {
		...
		if ((ff>0)&&(Being.feelsFear(a))) {
			if ((a.getStat(RPG.ST_WP)*(1+a.getStat(Skill.BRAVERY)))>=Rand.r(target.getStat(RPG.ST_WP)*ff)) {
	...
	public static void die(Thing t) {
		...
		t.remove();
		t.set("APS",-1000);
		t.set("HPS",Maths.min(0,t.getStat("HPS")));
	}
