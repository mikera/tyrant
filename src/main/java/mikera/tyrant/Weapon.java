...
public class Weapon {
    ...
    public static int calcASK(Thing w, Thing wielder, Thing target) {
        ...
        if (enc>0) {
        	enc=Maths.min(100,enc);
        	ASK=(ASK*(100-enc))/100;
        }
        
        ASK*=slayingModifier(w,target);
        
        return Maths.max(1,(int)Math.round(ASK));
    }
    ...
    public static int calcDSK(Thing t) {
        ...
        return Maths.max(0,(int)Math.round(dsk));
    }
    ...
    public static void addWithVariants(Thing w) {
        ...
        for (int i=0; i<wmat.length; i++) {
            ...
            t.set("LevelMin",Maths.max(1,levels[i]+w.getStat("LevelMin")));
