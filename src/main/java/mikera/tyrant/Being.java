...
public class Being {
	...
	public static boolean tryPickup(Thing h, Thing t) {
		...
		if (Item.isOwned(t)) {
			...
			// find value
			int val=Maths.max(1,Item.value(t));
	...
	public static void calcEncumberance(Thing b) {
		...
		// also trader bonus
		enc = Maths.middle(0, enc - (20 + 3 * b.getStat(Skill.TRADING)),100);
	...
	public static void heal(Thing b, int h) {
		...
		int gain = Maths.min(h, hpsmax - hps);
		if (gain > 0) {
			if (gain > hps) {
	...
	public static void recover(Thing t, int time) {
		...
		if (!hungry) {
			...
			t.set("HPS",Maths.middle(hps,hps+gain,hpsmax));

			int recharge = t.getStat(RPG.ST_RECHARGE);

			if (recharge > 0) {
				int mpsmax = t.getStat(RPG.ST_MPSMAX);
				t.set("MPS",Maths.middle(mps,mps+ RPG.round(((float) time* recharge * mpsmax) / 1000000),mpsmax));
			}
