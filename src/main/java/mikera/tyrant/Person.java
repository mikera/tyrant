...
public class Person {
		...
		public boolean handle(Thing t, Event e) {
			...
			if (h.getStat(sk) > 0) {
				if (get("Practitioner") != null) {
					Game.message("\"Good day to you, fellow "
							+ get("Practitioner") + ".\"");
				} else {
				...
				if (h.getStat(RPG.ST_SKILLPOINTS) >= 2) {
					if (Coin.getMoney(h) < price) {
						Game.message(t.getTheName() + " could teach you the "
								+ sk + " skill for " + Coin.valueString(price)
								+ "");
						...
						Game.message("Would you like to learn the " + sk
						...
						if (c == 'y') {
							...
							Game.message("You learn the " + sk + " skill from "
									+ t.getTheName());
						} else {
					...
					Game.message(t.getTheName() + " can teach the " + sk
							+ " skill for " + Coin.valueString(price) + "");
	...
	private static class ListSellingScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			for (Thing saleItem = Game.selectSaleItem("Select an item to sell:", h,
					t); saleItem != null; saleItem = Game.selectSaleItem(
					"Select an item to sell:", h, t)) 
			{
				if (saleItem.getFlag("IsMoney")) {
					continue;
				}

				int total = saleItem.getStat("Number");
				int count = 1;
				if (total > 1) {
					// count =
				...
				int value = Item.shopValue(saleItem, h, t, count);

				if ((saleItem.y > 0) && (!h.clearUsage(saleItem.y))) {
					continue;
				}

				Game.message("You sell your "
						+ saleItem.getName(Game.hero(), count) + " to "
						+ t.getTheName());
				if (count > 0) {
					saleItem.remove(count);
				}
		...
		public boolean handle(Thing t, Event e) {
			...
			if (damagedFlag) {
				...
				if (value > 0) {
					...
					char k = Game.getOption("ynq");
					if (k == 'y') {
						Game.message("You gave " + gift.getName(Game.hero())
		...
		public boolean handle(Thing t, Event e) {
			...
			if (t.toString().equals("jeweller")) {
				if (!gift.getFlag("IsRing") && !gift.getFlag("IsNecklace")) {
					Game.message("I am sorry, I only know about rings and necklaces.");
					return false;
				}
			}
			...
			char k = Game.getOption("ynq");
			if (k == 'y') {
				Game.message("You gave " + Coin.valueString(value) + " to the "
						+ t.toString() + " to identify the "
						+ gift.getName(Game.hero()) + ".");
				...
				// Identify the item
				Item.identify(gift);
				Game.message(t.getTheName() + " identifies it as "
						+ gift.getAName() + ".");
			} else {
				// you don't want to pay?
				giver.message("Don't waste my time then!");
	...
	private static class TeachingScript extends Script {
		...
		public boolean handle(Thing t, Event e) {
			...
			if (cost > 0) {
				Game.message(t.getTheName() + " asks for "
			...
			List<String> sks = Skill.getTrainableSkills(giver, true);
			List<String> sksr = Skill.getTrainableSkills(giver, false);
	...
	private static class RiddleScript extends Script {
		...
		private static ArrayList<String> riddles = null;
		...
		public boolean handle(Thing t, Event e) {
			...
			if (accept == 'y') {
				int riddleIndex = Rand.r(riddles.size());
				String riddle = "\"" + riddles.get(riddleIndex) + "\"";
				String answer = answers.get(riddleIndex);
				Game.message(riddle);
	...
	public static void initRaces() {
		...
		t.incStat(Skill.SMITHING, Rand.r(2));
		t.incStat(Skill.MINING, Rand.r(3));
		t.set("RaceDescription",
		...
		t.incStat(Skill.HERBLORE, Rand.r(3));
		t.incStat(Skill.COOKING, 1);
		t.incStat(Skill.STORYTELLING, Rand.r(2));
		t.set("RaceDescription",
		...
		t.incStat(Skill.TRADING, Rand.d(2));
		t.incStat(Skill.IDENTIFY, Rand.d(2));
		t.set("RaceDescription",
		...
		t.incStat(Skill.ATHLETICS, Rand.d(3));
		t.incStat(Skill.PERCEPTION, Rand.r(3));
		t.incStat(Skill.ALERTNESS, Rand.d(3));
		...
		t.incStat(Skill.LITERACY, Rand.d(2));
		t.incStat(Skill.IDENTIFY, Rand.r(2));
		t.incStat(Skill.RUNELORE, Rand.r(2));
	...
	public static void initWoodFolk() {
		...
		t.set("OnGift", SwapScript.create("IsCoin",
				(Rand.d(2) == 1) ? "IsScroll" : "IsIdentifyScroll", 6));
		Lib.add(t);
