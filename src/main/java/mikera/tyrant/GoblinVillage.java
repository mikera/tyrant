...
public class GoblinVillage {
	public static Map makeGoblinVillage() {
		...
		Point[] ps=new Point[Rand.d(2,10)];
		ps[0]=new Point(w/2,h/2);
		int count=1;
		for (int i=1; i<ps.length; i++) {
			for (int q=0; q<20; q++) {
				Point p=new Point(6+Rand.r(w-12),6+Rand.r(h-12));
				ps[i]=p;
				boolean found=true;
				for (int j=0; j<i; j++) {
		...
		for (int i=0; i<30; i++) {
			m.addThing(Lib.createType("IsGoblinoid",Rand.d(10)));
		}
		...
		for (int x=0; x<w; x++) {
			for (int y=0; y<h; y++) {
				...
				if (m.isBlank(x,y)&&(Rand.d(2500)<d)) {
	...
	public static void makeRandomFeature(Map m,int cx, int cy) {
		switch (Rand.d(10)) {
			...
			case 2: 
				...
				for (int x1=-2; x1<=2; x1++) {
					for (int y1=-2; y1<=2; y1++) {
						Thing gob=Lib.createType("IsGoblinoid",Rand.d(15));
				...
				for (int x1=-1; x1<=1; x1++) {
					for (int y1=-1; y1<=1; y1++) {
						if ((x1==0)&&(y1==0)) {
							...
							Thing gob=Lib.createType("IsGoblinoid",Rand.d(10));
							m.addThing(gob,cx+x1,cy+y1);
							AI.setGuard(gob,m,cx+x1,cy+y1);
						}
					}
			...
			default:
				m.fillOval(cx-2,cy-2,cx+2,cy+2,m.floor());
				Thing gob=Lib.createType("IsGoblinoid",10+Rand.d(10));
